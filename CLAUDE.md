...
Routes are file-based: `(tabs)` for the main shell (checklist index + reports/dashboard), modals for `profile`, `admin`, `settings`, `checklist-settings`, `history`, `import-pdf`, plus `login`, `create-account`, `onboarding`, and `+not-found`.
...
Checklist data (`ChecklistContext`) is backed by either the API or local AsyncStorage depending on `StorageMode` (`"cloud"` | `"local"` | `"local-no-auth"`), persisted under `STORAGE_MODE_KEY` in AsyncStorage. In cloud mode it uses `@workspace/api-client-react` hooks; in local modes it delegates every CRUD operation to the mirror functions in `utils/localChecklistStore.ts` instead. `"local"` means the user has a cloud account (created via `create-account.tsx`) but stores checklist data on-device; `"local-no-auth"` is the fully offline personal mode. In cloud mode, `utils/checklistCache.ts` (`ChecklistCache`) caches per-checklist data and active shift state in AsyncStorage under `@end_shift_v4_*` keys for offline resilience. Local app config (name, primaryColor, icon) and onboarding state still live in `AsyncStorage`. `CompletedChecklist` history shapes are kept for backward-compat but shift history now comes from the API (or local store in local modes).
