...
`artifacts/api-server/src/app.ts` mounts `pino-http`, `cors()`, JSON/urlencoded parsers, then the router from `routes/index.ts` (which combines `health`, `auth`, `profiles`, `roles`) under `/api`.

`index.ts` does two things before `app.listen`:
...
Auth model (see `lib/db/src/schema/auth.ts`):
- `roles` (with `level`, `isSystem`, `rights[]`) + `users` + `passkey_credentials` + `refresh_tokens`
- Rights enum: `manage_profiles`, `assign_roles`, `manage_roles`, `create_checklists`
